...
private[lsh] class PStableHashFamily(
  ...
  private[lsh] def initHashFamily: Array[PStableParameterSet] = {
    ...
    for (i <- 0 until familySize) {
      ...
      // step 2: select b, the original one has some problem
//      val b = uniformRandomizer.nextInt(w)
      val b = uniformRandomizer.nextDouble()*w
      // step 3: generate each hash function chain, original one is like the follows:
//      hashFamily(i) = new PStableParameterSet(vectorA, w, b)
      hashFamily(i) = new PStableParameterSet(vectorA, b, w)
...
//    for ( item <- chainedHashFunctions ) println(item.toString)
