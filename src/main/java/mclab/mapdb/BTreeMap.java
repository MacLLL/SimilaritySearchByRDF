...
public class BTreeMap<K, V>
  ...
  public int btreeCompareGroupLength = 0;
  public int btreeCompareGroupNum = 0;
  public int btreeMaximumNode = 0;
  ...
  private ValRef doUpdateOldValueRef(Object oldValue,
    ...
    if (oldValueRef.recids.size() >= btreeMaximumNode &&
            currentLevel < btreeCompareGroupNum - 1) {
      ...
      // redistribution
      // (just for generate similar workload but does not reproduce hash tree behavior exactly)
      Random r = new Random();
  ...
  private long calculateNextLevelHash(long completeHash, int currentLevel) {
    int totalHashBits = btreeCompareGroupNum * btreeCompareGroupLength;
    long nextLevel = currentLevel + 1;
    if (nextLevel >= btreeCompareGroupNum) {
      ...
      int nextShiftingLength = (btreeCompareGroupNum - 1 - (int) nextLevel) *
              btreeCompareGroupLength;
...
//            System.out.println(key+" - "+pos+" - "+A);
  ...
  private void appendExistingRecId(K newKey, long existingRecId, int proposedCurrentLevel) {
    K v = newKey;
    int currentLevel = Math.min(proposedCurrentLevel, btreeCompareGroupNum - 1);
    int stackPos = -1;
    long[] stackVals = new long[4];
