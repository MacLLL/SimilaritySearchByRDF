...
import static mclab.mapdb.DataIO.*;
...
public class StoreDirect extends Store {
   ...
   */
  //it rounds down, plus extra slot for zeros (not really used)
...
//            8*RECID_LAST_RESERVED;// also include reserved recids into mix;
  ...
  public StoreDirect(String fileName,
    ...
    super(fileName, volumeFactory, cache, lockScale, lockingStrategy, checksum, compress, password,
            readonly, snapshotEnable);
  ...
  protected void initCreate() {
    ...
    //Nan: the first page is the index page
    vol.ensureAvailable(PAGE_SIZE);
    vol.clear(0, PAGE_SIZE);

    //set sizes
    //Nan: pointing to the last allocated page inside the store
    vol.putLong(STORE_SIZE, parity16Set(PAGE_SIZE));
    //Nan: maximum allocated recid
    ...
    for (long masterLinkOffset = FREE_RECID_STACK;
         masterLinkOffset < HEAD_END;
         masterLinkOffset += 8) {
  ...
  protected void fillDataOutputByteArray(
          long[] offsets,
    ...
    try {
      if (offsets.length == 0) {
        ...
        int size = offsetsTotalSize(offsets);
        byte[] b = getLoadLinkedRecord(offsets, size);
        output.write(b);
      }
    } catch (IOException e) {
      e.printStackTrace();
  ...
  protected <A> A getFromOffset(Serializer<A> serializer, long[] offsets) {
    DataInput inputByteArray = null;
    int size = 0;
    if (offsets == null) {
      return null; //zero size
    } else if (offsets.length == 0) {
      inputByteArray = new DataInputByteArray(new byte[0]);
    } else if (offsets.length == 1) {
      //not linked
      size = (int) (offsets[0] >>> 48);
      long offset = offsets[0] & MOFFSET;
      inputByteArray = vol.getDataInput(offset, size);
    } else {
      //calculate total size
      size = offsetsTotalSize(offsets);
      byte[] b = getLoadLinkedRecord(offsets, size);
      inputByteArray = new DataInputByteArray(b);
    }
    return deserialize(serializer, size, inputByteArray);
  }

  protected byte[] getLoadLinkedRecord(long[] offsets, int totalSize) {
    //load data
  ...
  /**
   * Nan's comments: allocate (size) bytes from the volume
   */
  protected long[] freeDataTake(int size) {
    ...
    while (size > MAX_REC_SIZE) {
      ret = Arrays.copyOf(ret, ret.length + 1);
      //Nan's comments:
      //(((long) MAX_REC_SIZE) << 48) => points where long values are ending at current chunk
      //freeDataTakeSingle(round16Up(MAX_REC_SIZE)) =>
  ...
  protected void longStackPut(final long masterLinkOffset, final long value, boolean recursive) {
    ...
    if (currSize + 8 >= pageSize) {
      // +8 is just to make sure and is worse case scenario, perhaps make better check based on actual packed size
  ...
  /**
   * get a long integer from the stack
   *
   * @param masterLinkOffset pointing to the last chunk
   * @param recursive
   * @return the offset of the newly allocated memory space
   */
  protected long longStackTake(long masterLinkOffset, boolean recursive) {
    ...
    //Nan's comments:
    //tail pointer, points where long values are ending at current chunk. Its value changes on
    // every take/put
    long masterLinkVal = parity4Get(headVol.getLong(masterLinkOffset));
    if (masterLinkVal == 0) {
      return 0;
    }
    //Nan's comments: currSize => how many bytes have been occupied in this chunk
    long currSize = masterLinkVal >>> 48;
    //Nan's comments: the offset of the current chunk in the page
    //the last four bits indicate whether the chunk link to the next one
    final long pageOffset = masterLinkVal & MOFFSET;

    //read packed link from stack
    //Nan's comments: get 8 unsigned bytes from the volume
    ...
    //Nan's comments: IT'S A BUG should be PAGE_SIZE - currSize
    if (currSize > 8) {
     ...
     */
    long currentChunkMetadata = parity4Get(vol.getLong(pageOffset));
    ...
    freeDataPut(pageOffset, currentChunkSize);
          ...
          if (compactedFile == null) {
            ...
            if (!currFile.renameTo(currFileRenamed)) {
              ...
              throw new AssertionError("failed to rename file " + currFile + " - " +
                      currFile.exists() + " - " + currFileRenamed.exists());
            }
  ...
  protected void compactIndexPages(final long maxRecidOffset, final StoreDirect target,
                                   final AtomicLong maxRecid) {
  ...
  protected void compactIndexPage(long maxRecidOffset, StoreDirect target, AtomicLong maxRecid,
                                  int indexPageI) {
    final long indexPage = indexPages[indexPageI];

    long recid = (indexPageI == 0 ? 0 : indexPageI * (PAGE_SIZE - 8) / indexValSize -
            HEAD_END / indexValSize);
  ...
  protected void updateFromCompact(long recid, long indexVal, Volume oldVol) {
    //allocate new space
  ...
  protected long indexValGet(long recid) {
    ...
    long indexVal = vol.getLong(offset);
    if (indexVal == 0) {
      throw new DBException.EngineGetVoid(recid);
    }
    if (checksum) {
  ...
  /**
   * convert record id to the start offset of index page
   * @param recid the given record id
   * @return the offset of the starting offset of index page
   */
  protected final long recIdToIndexPageStartOffset(long recid) {
    ...
    long indexPageStartingAddress = (recid - 1) * indexValSize + HEAD_END + 8;
    ...
    indexPageStartingAddress += Math.min(1, indexPageStartingAddress / PAGE_SIZE) *
            (8 + ((indexPageStartingAddress - PAGE_SIZE) / (PAGE_SIZE - 8)) * 8);
    return indexPageStartingAddress;
  }

  /**
   * Comments from Nan Zhu: found the offset inside the index page from the recID
   */
  protected final long recidToOffset(long recid) {
    ...
    if (checksum) {
      //TODO: Nan Zhu: a bunch of duplicate in recIdToOffsetChecksum and recIdToOffset
      return recidToOffsetChecksum(recid);
    }

    long totalOffset = recIdToIndexPageStartOffset(recid);

    return indexPages[(int) (totalOffset / PAGE_SIZE)] + totalOffset % PAGE_SIZE;
  }
  ...
  /**
   * Nan's comments: padding the bytes to the multiple times of 16
   */
