...
import mclab.deploy.SamplingForestInit;
...
public class PartitionedHTreeMap<K, V>
  ...
  //Lu:serializer for key
  protected final Serializer<K> keySerializer;
  //Lu:serializer for value
  protected final Serializer<V> valueSerializer;
  //Lu:engines is the Map for every different engine, which keep different partitions.
  public final ConcurrentHashMap<Integer, Engine> engines = new ConcurrentHashMap<>();
  //Lu:snapshot for different partitions
  protected final ConcurrentHashMap<Integer, Engine> snapshots = new ConcurrentHashMap<>();
  protected final boolean closeEngine;
  // Lu:defalut value creater(used when there is no such key in the Map)
  protected final Fun.Function1<V, K> valueCreator;
  //Lu:RAM storage threshold
  protected final long ramThreshold;

  //Lu:controlling the simulation of default MapDB, indicate whether this Map is locked
  protected ReentrantReadWriteLock initStorageLock = new ReentrantReadWriteLock();
  //Lu: it indicate whether certain partition is initalized
  ...
  //Lu: locks for each partition
  protected HashMap<Integer, ReentrantReadWriteLock> structureLocks =
          new HashMap<Integer, ReentrantReadWriteLock>();

  //Lu: the counter for each partition
  protected final ConcurrentHashMap<Integer, Long[]> counterRecids =
          new ConcurrentHashMap<Integer, Long[]>();
  /**
   * list of segments, this is immutable, different Integer indicate the different partitions
   */
  /**
   * Lu's comment
   each partition corresponding to a long array which save the recid, recid is used to get the data from engine.
  ...
  //for persist indexing structure
   ...
   * @return no mean
   */
  public int getAllRootRecIdForEachSegment(){
  ...
  protected final Serializer<Object> DIR_SERIALIZER = new Serializer<Object>() {
    ...
    // Notes: Also, the last bit in recid indicates whether it is a leaf node(1) or dir node(0)
    public void serialize(DataOutput out, Object value) throws IOException {
      ...
      out2.packLong((((long) c[BITMAP_SIZE]) << 1) | 1L); //save the leaf node
  ...
  /**
   * In the experiment, BUCKET_LENGTH=28, So SEG is 16.
   * @param bucketLength
     */
  ...
  public int sizeOfMap(){
    ...
    for(int partitionId=0;partitionId<partitioner.numPartitions;partitionId++){
      Iterator iter=this.keySet(partitionId).iterator();
      while(iter.hasNext()){
        num++;
        iter.next();
      }
...
//    System.out.println(counterRecids==null);
    if (counterRecids != null) {
      long ret = 0;
   ...
   * @return
   */
  public LinkedList<K> multiProbeGetSimilar(
          final Object key, int seed){
    final int h = hash((K) key);
    final int seg = h >>> BUCKET_LENGTH;
...
//    return filter(lns,h);
  ...
  protected final int dirOffsetFromSlot(int[] dir, int slot) {
    ...
    //rangeDecidingBits indicates where the recid should put in BITMAP,
    int bitmapRange = 0;
    if (BITMAP_SIZE > 1) {
  ...
  /**
   * init Partition, every time ,it needs to use 8 long
   * @param partitionId
   */
  private void initPartition(int partitionId) {
    // to simulate the default mapdb setup
    // all partition shares the same store with the lockscale of 16, since we only have 16 segments which is default setup in concurrentHashMap
//    System.out.println("initialize partition:"+partitionId);
...
//        }
  ...
  public int hash(final K key) {
    if (hasher instanceof LocalitySensitiveHasher) {
      ...
      System.out.println(SamplingForestInit.createFlag());
      if (SamplingForestInit.createFlag() == true ){
        SparseVector v = SamplingForestInit.vectorIdToVector().get(key);
        if (v == null) {
...
//      }
  ...
  public void getPartitionNums(){
    for (int i = 0; i < times.length; i++) {
      System.out.println("Partition "+i+" has "+ times[i]+" objects.");
    }
  }
...
//      System.out.println("before lock"+partitionRamLock.get(partition)[seg]);
      partitionRamLock.get(partition)[seg].writeLock().lock();
//      System.out.println("after lock"+partitionRamLock.get(partition)[seg]);
...
//      System.out.println("before release lock"+partitionRamLock.get(partition)[seg]);
      partitionRamLock.get(partition)[seg].writeLock().unlock();
//      System.out.println("after release lock"+partitionRamLock.get(partition)[seg]);
    }
    ...
    while (true) {
      ...
      if (bucketConflictCost >= BUCKET_OVERFLOW && level >= 1) {
        Object newDirNode = new int[BITMAP_SIZE];
        {
          ...
          //update the dir node with the new LinkedNode, make the last bit as 1
          //to indicate the directory node
//          newDirNode = putNewRecordIdInDir(newDirNode, pos, (newRecid << 1) | 1);
          newDirNode = putNewRecordIdInDir(newDirNode, pos, (newRecid << 1) | 0);
        }
        ...
        //Lu's: because the last bit label the type of node
...
//                        counter++;
  ...
  public void initStructureLocks() {
    System.out.println("run the PartitionedHTreeMap initStructureLocks");
