...
class UnsafeStuff {
  ...
  static final class UnsafeVolume extends Volume {
    ...
    private static final long ARRAY_BASE_OFFSET = UNSAFE == null ? -1 :
            UNSAFE.arrayBaseOffset(byte[].class);
    ;

    public static final VolumeFactory FACTORY = new VolumeFactory() {
      @Override
      public Volume makeVolume(String file, boolean readOnly, int sliceShift, long initSize,
                               boolean fixedSize) {
    ...
    volatile sun.nio.ch.DirectBuffer[] buffers = new sun.nio.ch.DirectBuffer[0];
    ...
    public void putByte(long offset, byte value) {
      ...
      //*LOG*/ System.err.flush();
      //Nan: offset >>> sliceShift is the index to the memory page
      final long address = addresses[((int) (offset >>> sliceShift))];
      offset = offset & sliceSizeModMask;
