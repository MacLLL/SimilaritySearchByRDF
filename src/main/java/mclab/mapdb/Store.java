...
public abstract class Store implements Engine {
  ...
  protected void checkFeaturesBitmap(final long feat) {
    ...
    if (xteaEnc && !encrypt) {
      throw new DBException.WrongConfig("Store was created with encryption, " +
              "but no password is set in config.");
    }
    if (!xteaEnc && encrypt) {
    ...
    if (lzwComp && !compress) {
      throw new DBException.WrongConfig("Store was created with compression, " +
              "but no compression is enabled in config.");
    }
    if (!lzwComp && compress) {
      throw new DBException.WrongConfig("Compression is set in config, " +
              "but store was created with compression.");
    }

    boolean crc = (feat >>> FEAT_CRC & 1) != 0;
    if (crc && !checksum) {
      throw new DBException.WrongConfig("Store was created with CRC32 checksum, " +
              "but it is not enabled in config.");
    }
    if (!crc && checksum) {
    ...
    if (endZeroes < FEAT_CRC) {
      throw new DBException.WrongConfig("Unknown feature #" + endZeroes +
              ". Store was created with never MapDB version, " +
  ...
  protected Store persist(String filePath) {
    return null;
  }
  ...
  protected <A> A deserialize(Serializer<A> serializer, int size, DataInput input) {
    try {
      ...
      if (size + start > di.getPos())
        throw new DBException.DataCorruption("Data were not fully read, check your serializer. " +
                "Read size:" + (di.getPos() - start) + ", expected size:" + size);
      if (size + start < di.getPos())
        throw new DBException.DataCorruption("Data were read beyond record size, check your " +
                "serializer. Read size:" + (di.getPos() - start) + ", expected size:" + size);
   ...
   */
  private <A> A deserializeExtra(Serializer<A> serializer, int size, DataIO.DataInputInternal di)
          throws IOException {
    if (checksum) {
  ...
  @Override
  public <A> boolean compareAndSwap(long recid, A expectedOldValue, A newValue,
    ...
    try {
      ...
      if (oldVal == expectedOldValue || (oldVal != null && serializer.equals(oldVal,
              expectedOldValue))) {
...
//        }
  ...
  protected void assertWriteLocked(int segment) {
    ...
    if (l instanceof ReentrantReadWriteLock &&
            !((ReentrantReadWriteLock) l).isWriteLockedByCurrentThread()) {
      throw new AssertionError();
    }
  }
  ...
  public static abstract class Cache {
     ...
     * It checks free memory every N operations (1024*10). If free memory is bellow 75% it clears
     * the cache
     *
     * @author Jan Kotek
     */
    public static final class HardRef extends Store.Cache {
      ...
      public HardRef(int initialCapacity, boolean disableLocks, ScheduledExecutorService executor,
                     long executorPeriod) {
        super(disableLocks);
  ...
  /**
   * Lock which blocks parallel execution, but does not use MemoryBarrier (and does not flush CPU
   * cache)
   */
  public static final class MemoryBarrierLessLock implements Lock {
    ...
    public void unlock() {
      ...
      if (!lockedThread.compareAndSet(hash, Long.MAX_VALUE)) {
        throw new IllegalMonitorStateException("Can not unlock, current thread does not " +
                "hold this lock");
      }
    }
