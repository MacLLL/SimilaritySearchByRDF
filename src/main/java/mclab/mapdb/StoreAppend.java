...
/**
 * append only store, in persist memory
 */
public class StoreAppend extends Store {
  ...
  private Serializer dataSummarySerializer;
  ...
  protected StoreAppend(String fileName,
                        ...
                        boolean txDisabled,
                        ScheduledExecutorService compactionExecutor,
                        Serializer dataSummarySerializer) {
    this(fileName, volumeFactory, cache, lockScale, lockingStrategy, checksum, compress,
            password, readonly, snapshotEnable, txDisabled, compactionExecutor);

    this.dataSummarySerializer = dataSummarySerializer;
  }
  ...
  public void init() {
    ...
    try {
      ...
      indexTable = new Volume.FileChannelVol(new File(this.fileName + "-index"));
      if (!readonly) {
  ...
  private void updateLongLongMap(LongLongMap map, long recid, long value) {
    if (map != null) {
      map.put(recid, value);
    }
  }

  void extendSpace(long maxRecId) {
    indexTable.ensureAvailable(maxRecId * 8);
  }
    ...
    try {
      ...
      while (true) {
        ...
        if (inst == I_INSERT || inst == I_UPDATE) {
          ...
          updateLongLongMap(commitData, recid, instPos);
          ...
          updateLongLongMap(commitData, recid, -1);
  ...
  /**
   * update eof for new entry
   * @param headSize the size of metadata
   * @param totalSize the total size of entry
   * @return the starting address of the new allocated entry,
   */
  protected long alloc(int headSize, int totalSize) {
    structuralLock.lock();
    try {
      // Nan Zhu: skip the instruction byte
      // whether we need to allocate new bytes???
  ...
  public void persistDataSummary() {
    try {
      if (bloomFilter == null) {
        throw new IOException("cannot persist bloom filter before it is initialized");
      }
      DataOutputStream out = new DataOutputStream(new BufferedOutputStream(
              new FileOutputStream(fileName + "-summary")));
  ...
  public boolean searchInDataSummary(Object key) {
    //search in data summary
    try {
      BloomFilter dataSummary = BloomFilter.readFrom(
  ...
  protected <A> A get2(long recid, Serializer<A> serializer) {
    ...
    long offset = 0L;
    if (tx) {
      LongLongMap longLongMap = modified[lockPos(recid)];
      offset = (longLongMap != null) ? longLongMap.get(recid) : 0;
    }
   ...
   * @param recid the record id of the entry
   * @param out the data stream representing the object
   * @param isInsert if inserting the new entry
   */
  void insertOrUpdate(long recid, DataIO.DataOutputByteArray out, boolean isInsert) {
    if (CC.ASSERT)
      assertWriteLocked(lockPos(recid));

    //TODO assert indexTable state, record should already exist/not exist
    //Nan Zhu: the payload size of the entry
    final int payloadSize = out == null ? 0 : out.pos;
    final int shiftedSize = out == null ? 0 : payloadSize + 1;  //one additional state to indicate null
    //Nan Zhu: metadata size for the entry
    ...
    long offset = alloc(headSize, headSize + payloadSize);
    final long origOffset = offset;
    //ensure available worst case scenario
    vol.ensureAvailable(offset + headSize + payloadSize);
    //instruction
    ...
    indexTablePut(recid, out == null ? -3 : (payloadSize == 0) ? -2 : origOffset);
