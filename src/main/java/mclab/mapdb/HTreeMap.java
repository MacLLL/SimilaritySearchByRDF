    ...
    private V putInner(K key, V value, int h, int segment) {
        ...
        while(true){
            ...
            if (bucketConflictCost >= BUCKET_OVERFLOW && level >= 1) {
                ...
                final long expireNodeRecid = expireFlag? engine.put(ExpireLinkNode.EMPTY,
                        ExpireLinkNode.SERIALIZER): 0L;
    ...
    private long expireCalcRemovePerSegment() {
        ...
        if(expireMaxSizeFlag){
            ...
            if(size>expireMaxSize){
                ...
                if(LOG.isLoggable(Level.FINE)){
                    LOG.log(Level.FINE,
                            "HTreeMap expirator expireMaxSize, will remove {0,number,integer} entries per segment",
                            removePerSegment);
                }
            }
        }
...
//            expireCheckSegment(seg);
     ...
     * modifications made by other threads. Useful if you need consistent view on Map.
     * </p><p>
