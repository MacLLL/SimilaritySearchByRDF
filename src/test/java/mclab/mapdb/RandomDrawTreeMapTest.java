...
public class RandomDrawTreeMapTest {
  ...
  public void testDirSerializer() throws IOException {
    ...
    for (int slot = 0; slot < 128; slot += 1 + slot / 5) {
      dir = m.putNewRecordIdInDir(dir, slot, slot * 1111);
    }

//    int[] updatedir=(int[])dir;
//    for (int i=0; i<updatedir.length;i++){
//      System.out.println(updatedir[i]);
//    }
...
//    System.out.println(out.buf.toString());
    ...
    for (int slot = 0; slot < 128; slot += 1 + slot / 5) {
      ...
      org.junit.Assert.assertEquals(slot * 1111, RandomDrawTreeMap.dirGet(dir2, offset));
    }
  }

  /**
   * swap the DataOutputByteArray to DataInputByteBuffer
   * @param d
   * @return
     */
  ...
  public void ln_serialization() throws IOException {
    ...
    RandomDrawTreeMap.LinkedNode n = new RandomDrawTreeMap.LinkedNode(123456, 123L, 456L);
    ...
    RandomDrawTreeMap.LinkedNode n2 =
            (RandomDrawTreeMap.LinkedNode) m.LN_SERIALIZER.deserialize(in, -1);
  ...
  public void test_hash_collision() {
    ...
    RandomDrawTreeMap m = new RandomDrawTreeMap(0,
            "default",
            tempDirFile.toString(),
            "RandomDrawTreeMap",
            partitioner,
  ...
  public void test_hash_dir_expand() {
    ...
    System.out.println(m.engines.get(0));
    ...
    long recid = l[4];
    System.out.println("recid=l[4]=" + recid);
//    RandomDrawTreeMap.LinkedNode ln=(RandomDrawTreeMap.LinkedNode)engine.get(recid >>> 1,m.LN_SERIALIZER);
//    System.out.println("key is " + ln.key +"value is "+ ln.value);
//    m.getAllRootRecIdForEachSegment();
    org.junit.Assert.assertEquals(1, recid & 1);  //last bit indicates linkedlist
    // hash always be 0, so only the first bit is 1
    org.junit.Assert.assertEquals(1, l[0]);
    //all others should be null
    for (int i = 1; i < 4; i++)
      org.junit.Assert.assertEquals(0, l[i]);

    //from the engine, we have to >>>1 of recid to get from engine
    //if within a linkedlist, we don't have to >>> 1 recid, since we know it's all the node
    recid = recid >>> 1;

    for (int i = m.BUCKET_OVERFLOW - 1; i >= 0; i--) {
      org.junit.Assert.assertTrue(recid != 0);
      RandomDrawTreeMap.LinkedNode n =
              (RandomDrawTreeMap.LinkedNode) engine.get(recid, m.LN_SERIALIZER);
      System.out.println(i+"'s key is "+n.key+", " + i+"'s value is "+n.value);
    ...
    //take the recid of index root recid
    ...
    recid = l[4];
    //prove it's a d-node
    org.junit.Assert.assertEquals(0, recid & 1);  //last bite indicates dir
    org.junit.Assert.assertEquals(1, l[0]);
...
//
    ...
    recid = recid >>> 1;
    //after we know it's a linkedlist, take the node from linkerlist
    for (int i = 0; i <= m.BUCKET_OVERFLOW; i++) {
      org.junit.Assert.assertTrue(recid != 0);
      RandomDrawTreeMap.LinkedNode n =
              (RandomDrawTreeMap.LinkedNode) engine.get(recid, m.LN_SERIALIZER);
  ...
  public void hasher() {
    ...
    RandomDrawTreeMap m = new RandomDrawTreeMap<int[], Object>(
            0,
            "default",
            tempDirFile.toString(),
            "RandomDrawTreeMap",
  ...
  public void test_iterate_and_remove() {
    ...
    RandomDrawTreeMap m1 = new RandomDrawTreeMap(
            0,
            "default",
            tempDirFile.toString(),
            "RandomDrawTreeMap",
  ...
  public void testValueCreater() {
    ...
    final RandomDrawTreeMap m = new RandomDrawTreeMap(
            0,
            "default",
            tempDirFile.toString(),
            "RandomDrawTreeMap",
  ...
  public void slot_to_offset_long() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      for (int slot = 0; slot < 128; slot++) {
        ...
        int coffset = RandomDrawTreeMap.dirOffsetFromSlot(l, slot);
    ...
    for (int a = 0; a < 100; a++) {
      ...
      for (int i = 0; i < 1e3; i++) {
        ...
        for (int j = 0; j < 127; j++) {
          ...
          if (offset > 0)
            dir2[j] = RandomDrawTreeMap.dirGet(dir, offset);
        }
